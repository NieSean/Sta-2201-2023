...
Expected deaths is the implied number of lip cancer deaths for a particular region given that regions age structure and national level age- specific mortality rates for lip cancer. for example, and expected number of deaths would mean for that particular region we would expect 6 lip cancer deaths if this region were to experience the same age specific mortality rate as at the national level.
...
- Showing the first 5 $\alpha_i$ in the summary below:
...
res |>
  ...
  labs(title = "Distribution Of Log Theta", x = "Log theta")
```
...
res |>
  ...
  labs(title = "Log Theta vs Log Smr", x = "Log smr", y = "Log theta")
```
