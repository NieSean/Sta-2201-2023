...
-   \textcolor{blue}{The lab questions and solutions are are the end of this lab document.}
...
dm |>
  ...
  geom_line() +
  labs(title = 'male/female mortality ratio over time',
...
dm |>
  ...
  drop_na() |>
  mutate(f_count = Female.x * Female.y,
         m_count = Male.x * Male.y) |>
  group_by(Year) |>
  summarise(rate_female = sum(f_count)/sum(Female.y),
            rate_male = sum(m_count)/sum(Male.y)) |>
  pivot_longer(rate_female:rate_male, 
               names_to = 'sex', 
               values_to = 'weighted_rate') |>
  ggplot(aes(x = Year, y = weighted_rate, color = sex)) +
  geom_line() +
  labs(title = 'mortality rate over year',
       subtitle = "weighted average & by sex",
       y = 'weighted averagr rate') +
  theme_bw(base_size = 14) +
  scale_color_brewer(palette = "Set1")

```

Male have significantly higher mortality rate than female, until around year 200 when the two became close.
